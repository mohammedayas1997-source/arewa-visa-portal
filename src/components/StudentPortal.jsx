...
const StudentPortal = () => {
      ...
      try {
        ...
        const unsubUser = onSnapshot(userRef, (snap) => {
          if (snap.exists()) {
            ...
            const courseStartDate = data.courseSelectionDate?.toDate() || new Date();
  ...
  const handleImageUpload = async (e) => {
    ...
    setLoading(true);
    try {
      const storageRef = ref(storage, `profiles/${studentData.id}_${Date.now()}`);
      await uploadBytes(storageRef, file);
      const downloadURL = await getDownloadURL(storageRef);
      await updateDoc(doc(db, "users", studentData.id), { photoURL: downloadURL, lastUpdated: serverTimestamp() });
      setSettingsMessage({ type: "success", text: "Profile identity updated." });
    } catch (err) {
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    try {
      ...
      setSettingsMessage({ type: "success", text: "Security credentials updated." });
      setPasswords({ current: "", new: "", confirm: "" });
    } catch (err) {
      setSettingsMessage({ type: "error", text: "Security re-auth failed." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExamSubmit = async () => {
    ...
    const resultData = { score: finalScore, correctAnswers: correctCount, totalQuestions: questions.length, passed: finalScore >= 50, timeCompleted: serverTimestamp(), courseId: selectedCourseId, week: currentWeek };
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center font-black text-blue-600 bg-slate-950">
      ...
      <p className="tracking-widest animate-pulse uppercase">Syncing Arewa Visa Academy Protocols...</p>
    </div>
  );
    ...
    <div className={`min-h-screen flex font-sans ${darkMode ? "bg-slate-950 text-white" : "bg-gray-50 text-slate-900"} transition-colors duration-300`}>
      <aside className={`fixed lg:sticky top-0 z-50 h-screen w-80 border-r flex flex-col transition-transform ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-gray-100"} ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="p-10 flex flex-col gap-4">
          <h1 className="text-2xl font-black text-blue-600 italic">AREWA <span className={darkMode ? "text-white" : "text-gray-900"}>VISA ACADEMY</span></h1>
          <div className="p-4 rounded-2xl bg-blue-600/10 border border-blue-600/20">
            <div className="flex items-center gap-3 text-blue-500 mb-1"><Clock size={14} className="animate-spin-slow" /><span className="text-[10px] font-black uppercase tracking-widest">System Time</span></div>
        ...
        <nav className="flex-1 px-6 space-y-2 overflow-y-auto custom-scrollbar">
          {[{ id: "dashboard", name: "Dashboard", icon: <LayoutDashboard size={18} /> }, { id: "courses", name: "Curriculum", icon: <BookOpen size={18} /> }, { id: "discussions", name: "Forum", icon: <MessageSquare size={18} /> }, { id: "library", name: "Library", icon: <BookOpen size={18} /> }, { id: "chat", name: "Direct Link", icon: <User size={18} /> }, { id: "settings", name: "Settings", icon: <Settings size={18} /> }].map((item) => (
            <button key={item.id} onClick={() => { setActiveTab(item.id); setViewState("list"); setMobileMenuOpen(false); }} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === item.id ? "bg-blue-600 text-white shadow-lg" : "text-gray-400 hover:bg-blue-50/10"}`}>{item.icon} {item.name}</button>
          ))}
      ...
      <main className="flex-1 p-6 md:p-14 overflow-y-auto">
        <header className="lg:hidden flex justify-between items-center mb-10"><button onClick={() => setMobileMenuOpen(true)} className="p-3 bg-blue-600 text-white rounded-xl"><Menu /></button><h2 className="font-black italic uppercase text-blue-600">AREWA VISA</h2></header>
        ...
        {activeTab === "settings" && (
          <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
            <div className="flex items-center gap-4 border-b border-slate-200 dark:border-slate-800 pb-8"><div className="p-3 bg-blue-600 text-white rounded-2xl shadow-lg"><ShieldCheck size={28} /></div><div><h1 className="text-3xl font-black uppercase italic tracking-tighter">Security <span className="text-blue-600">& Profile</span></h1><p className="text-[10px] font-black uppercase tracking-widest opacity-40">UID: {studentData?.id}</p></div></div>
            {settingsMessage.text && <div className={`p-5 rounded-2xl flex items-center gap-3 font-black text-[10px] uppercase border animate-in zoom-in ${settingsMessage.type === "success" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-500" : "bg-red-500/10 border-red-500/20 text-red-500"}`}>{settingsMessage.type === "success" ? <CheckCircle2 size={18} /> : <AlertCircle size={18} />}{settingsMessage.text}</div>}
            <div className="grid md:grid-cols-3 gap-12">
              <div className="space-y-6"><h3 className="text-[10px] font-black uppercase tracking-widest text-blue-600">Profile Identity</h3><div className="relative"><div className="w-full aspect-square rounded-[3rem] overflow-hidden border-4 border-white/10 shadow-2xl bg-slate-800 flex items-center justify-center relative">{studentData?.photoURL ? <img src={studentData.photoURL} className="w-full h-full object-cover" alt="Profile" /> : <User size={60} className="text-slate-700" />}</div><label className="absolute -bottom-4 -right-4 p-4 bg-blue-600 text-white rounded-2xl shadow-xl cursor-pointer hover:scale-110 transition-all"><Camera size={20} /><input type="file" className="hidden" accept="image/*" onChange={handleImageUpload} disabled={loading} /></label></div></div>
              <div className="md:col-span-2 space-y-8"><h3 className="text-[10px] font-black uppercase tracking-widest text-blue-600">Password Encryption</h3><form onSubmit={handlePasswordUpdate} className={`p-10 rounded-[2.5rem] border shadow-xl space-y-6 ${darkMode ? "bg-slate-900 border-white/5" : "bg-white border-gray-100"}`}><input type="password" placeholder="CURRENT PASSWORD" required className="s-input" value={passwords.current} onChange={(e) => setPasswords({ ...passwords, current: e.target.value })} /><div className="grid grid-cols-2 gap-4"><input type="password" placeholder="NEW PASSWORD" required className="s-input" value={passwords.new} onChange={(e) => setPasswords({ ...passwords, new: e.target.value })} /><input type="password" placeholder="CONFIRM NEW" required className="s-input" value={passwords.confirm} onChange={(e) => setPasswords({ ...passwords, confirm: e.target.value })} /></div><button disabled={loading} className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3">{loading ? <RefreshCcw className="animate-spin" /> : <UploadCloud size={16} />} Update Credentials</button></form></div>
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-6">
            {libraryLinks.map((lib, i) => (
              <a key={i} href={lib.url} target="_blank" rel="noreferrer" className={`p-8 rounded-[2.5rem] border-2 transition-all hover:scale-105 group ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-white shadow-xl"}`}>
                <div className="flex justify-between items-start mb-6"><div className="p-3 bg-blue-600/10 text-blue-600 rounded-xl group-hover:bg-blue-600 transition-colors"><BookOpen size={24} /></div><span className="text-[10px] font-black uppercase text-blue-500 bg-blue-500/10 px-3 py-1 rounded-full">{lib.cat}</span></div>
                <h3 className="text-xl font-black uppercase italic mb-4">{lib.name}</h3>
                <div className="flex items-center gap-2 text-[10px] font-black uppercase text-gray-500 group-hover:text-blue-600">Open Resource <ChevronRight size={14} /></div>
              </a>
            ))}
          </div>
        )}
        ...
        {activeTab === "chat" && (
          <div className={`h-[80vh] flex flex-col rounded-[3.5rem] border overflow-hidden ${darkMode ? "bg-slate-900 border-white/5" : "bg-white shadow-2xl"}`}>
            <div className="p-8 bg-blue-600 text-white flex justify-between items-center shadow-lg"><h3 className="text-2xl font-black uppercase italic tracking-tighter">Secure Instructor Link</h3><ShieldCheck size={30} /></div>
            <div className="flex-1 p-8 overflow-y-auto space-y-4 flex flex-col custom-scrollbar">{privateMessages.map((m) => (<div key={m.id} className={`max-w-[75%] p-5 rounded-[2rem] text-sm font-bold shadow-sm ${m.senderRole === "student" ? "bg-blue-600 text-white self-end rounded-tr-none" : "bg-slate-800 text-white self-start rounded-tl-none border border-white/5"}`}>{m.text}<div className="text-[8px] opacity-40 mt-2 uppercase">{m.sender} • {m.createdAt ? formatDate(m.createdAt) : "Transmitting..."}</div></div>))}</div>
            <form onSubmit={async (e) => { e.preventDefault(); if (!newPrivateMsg.trim()) return; await addDoc(collection(db, "private_chats"), { text: newPrivateMsg, sender: studentData?.fullName || "Student", senderRole: "student", studentId: studentData?.id, createdAt: serverTimestamp() }); setNewPrivateMsg(""); }} className="p-6 border-t border-white/5 flex gap-4 bg-slate-900/10"><input value={newPrivateMsg} onChange={(e) => setNewPrivateMsg(e.target.value)} className="flex-1 bg-transparent border-none outline-none font-black text-sm" placeholder="Message instructor..." /><button className="p-5 bg-blue-600 text-white rounded-2xl shadow-xl hover:scale-105 transition-all"><Send size={20} /></button></form>
          </div>
        )}
      </main>
