...
const CourseEnrollment = ({ showCourseForm, setShowCourseForm, coursesData }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // Uploading Optional Docs if they exist
  ...
  return (
    <div className="fixed inset-0 bg-[#F0F4F8] z-[10000] overflow-y-auto pt-4 pb-20 px-4 md:px-20 animate-in fade-in duration-500">
      <div className="max-w-5xl mx-auto bg-white shadow-2xl rounded-[40px] overflow-hidden border border-slate-100 relative mb-10">
        <button onClick={() => setShowCourseForm(false)} className="absolute top-6 right-6 z-50 bg-red-600 hover:bg-[#002147] p-2 rounded-full text-white shadow-lg transition-all">
            <X size={24} />
        </button>
        ...
        <div className="bg-gradient-to-r from-[#002147] to-[#003366] p-12 text-white flex justify-between items-center relative overflow-hidden border-b-8 border-red-600">
          <div className="z-10 flex items-center gap-6">
            ...
            <div>
              <h1 className="text-4xl font-black uppercase tracking-tighter">Admission Portal</h1>
        ...
        <form onSubmit={handleFormSubmit} className="p-8 md:p-14 space-y-12 text-left">
          ...
          {/* OPTIONAL DOCUMENTS - NEW SECTION */}
          ...
          <button disabled={loading} type="submit" className="w-full bg-red-600 text-white font-black py-8 rounded-[2.5rem] uppercase tracking-[0.2em] shadow-2xl hover:bg-[#002147] transition-all flex items-center justify-center gap-4 transform hover:scale-[1.02] active:scale-95">
            {loading ? <Loader2 className="animate-spin" /> : <>Complete Enrollment <ArrowRight /></>}
          </button>
        </form>
      </div>
