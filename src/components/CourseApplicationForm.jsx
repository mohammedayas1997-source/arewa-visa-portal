...
const CourseApplicationForm = ({
  ...
  const uploadFile = async (file, path) => {
    ...
    // CRITICAL FIX: Await the upload and use snapshot.ref
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Remove file objects before Firestore storage
      ...
      window.scrollTo(0, 0); // Reset scroll to see payment UI
    } catch (error) {
  ...
  // --- STEP 2: AUTOMATIC UPDATE AFTER PAYMENT SUCCESS ---
  const handlePaymentSuccess = async (reference) => {
    ...
    try {
      ...
      // CRITICAL: Switch to Success view immediately
      setIsSuccess(true); 
      ...
      alert("Payment was successful, but database update failed. Please screenshot this screen.");
    } finally {
      setIsSubmitting(false);
    }
  };
